...
def transform_cam_to_world(P_c):
    ...
    # print('CAM_POSE', qx, qy, qz, qw, X_w, Y_w, Z_w)
...
class DroneCommNodeTask4(Node):
    ...
    def realsense_callback(self, msg):
        ...
        CAM_POSE = q_new[1], q_new[2], q_new[3], q_new[0], msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z
    ...
    def handle_launch(self, request, response):
        ...
        self.hover_pose.pose.position.z = HEIGHT
    ...
    def publish_waypoints(self):
        ...
        if CAM_POSE:
            for x, y, z in waypoints:
                ...
                x, y, z = transform_cam_to_world(point_in_camera)

                pose = Pose()
                pose.position.x = x
                pose.position.y = y
                pose.position.z = HEIGHT
                self.get_logger().info(f"Target {x:.3f} {y:.3f} {z:.3f} {HEIGHT}")
